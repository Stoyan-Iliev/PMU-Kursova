...
public class AddModifyTask extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateText.setText(new SimpleDateFormat("E, dd MMMM yyyy")
    ...
    public void init_modify() {
        ...
        if (task != null) {
            ...
            try {
                calendar.setTime(Objects.requireNonNull(iso8601Format
                        .parse(task.getString(2))));
    ...
    public void saveTask(View v) {
        if (edit_text.getText().toString().trim().length() > 0) {
            if (isModify) {
                myDB.updateTask(task_id, edit_text.getText().toString(),
